...
public class TeacherController {
	...
	public ResponseEntity<List<RegistrationDTO>> findAllScheduleByTeacher(@RequestBody TeacherHistoryRequest t){
		List<RegistrationDTO> registrationDTOList = this.registrationService.findAllByTeacher(t);
		for (int i = 0; i<registrationDTOList.size();i++) {
			if(!registrationDTOList.get(i).isEnable()){
				registrationDTOList.remove(i);
			}
		}
		return new ResponseEntity<>(registrationDTOList, HttpStatus.OK);
	}
